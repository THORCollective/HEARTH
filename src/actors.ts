...
async function loadAll(): Promise<LoadedData | null> {
  try {
    const [hunts, graph, mentions, matrix] = await Promise.all([
    ...
    return { hunts, graph, mentions, matrix };
  } catch (err) {
...
function pickPlaceholderExamples(index: ActorIndex): string[] {
  const pool = [...POPULAR_ACTOR_NAMES, ...POPULAR_ACTOR_ALIASES].filter(
    (slug) => resolveActor(slug, index, 1).length > 0,
  );
...
function renderShowcase(state: PageState) {
  ...
  if (bestEl) {
    bestEl.innerHTML = best.map((row, i) => rowForBest(row, i === 0)).join("");
  }
  if (gapsEl) {
    gapsEl.innerHTML = gaps.map((row, i) => rowForGap(row, i === 0)).join("");
  }
  root.hidden = false;
}

function rowForBest(row: ActorLeaderRow, isTop: boolean): string {
  const id = row.actor.id.replace("actor:", "");
  const covered = row.coverage.techniquesCovered;
  const total = row.coverage.actorTechniqueCount;
  // Bar fills toward fully covered — higher coverage reads as a bigger bar.
  const pct = Math.max(8, row.coverage.coveragePercent);
  return `<li${isTop ? ' class="top-rank"' : ""}>
    <a class="actor-name" href="?actor=${encodeURIComponent(id)}">
      ...
      <span class="small">${covered} of ${total} techniques hunted</span>
    </a>
    <span class="bar-cell"><span class="track"></span><span class="fill" style="width:${pct.toFixed(1)}%"></span></span>
    <span class="metric">${row.coverage.coveragePercent}<span class="den">% covered</span></span>
  </li>`;
}

function rowForGap(row: ActorLeaderRow, isTop: boolean): string {
  const id = row.actor.id.replace("actor:", "");
  const gap = row.coverage.gapTechniqueCount;
  const hunts = row.coverage.matchedHuntCount;
  // Bar fills toward fully uncovered — lower coverage reads as a bigger bar.
  const pct = Math.max(8, 100 - row.coverage.coveragePercent);
  return `<li${isTop ? ' class="top-rank"' : ""}>
    <a class="actor-name" href="?actor=${encodeURIComponent(id)}">
      <strong>${escapeHtml(row.actor.label)}</strong>
      <span class="small">${gap} ${gap === 1 ? "technique" : "techniques"} to hunt · ${hunts} ${hunts === 1 ? "hunt" : "hunts"}</span>
    </a>
