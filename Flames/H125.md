...
An adversary is using InstallUtil.exe with the /u (uninstall) flag to proxy-execute a malicious .NET assembly from a Temp directory, bypassing application whitelisting controls.

| Hunt # | Hypothesis | Tactic | Notes | Tags | Submitter |
|--------|-----------|--------|-------|------|-----------|
| H125 | An adversary is using InstallUtil.exe with the /u (uninstall) flag to proxy-execute a malicious .NET assembly from a Temp directory, bypassing application whitelisting controls. | Defense Evasion | Hunt for InstallUtil.exe process creation (Sysmon Event ID 1) with the /u flag targeting DLL or EXE files in %TEMP%, %APPDATA%, or other user-writable directories. Monitor for InstallUtil.exe making outbound network connections (Sysmon Event ID 3) — legitimate installations rarely require network access. Correlate with preceding csc.exe compilations in Temp directories, which indicate dynamic .NET assembly generation before proxy execution. Look for InstallUtil.exe executed outside of software deployment windows. | #defense_evasion #T1218_009 #installutil #lolbin #proxy_execution | [Lauren Proehl](https://x.com/jotunvillur) |
...
- [MITRE ATT&CK T1218.009 - System Binary Proxy Execution: Regsvcs/Regasm](https://attack.mitre.org/techniques/T1218/009/)
