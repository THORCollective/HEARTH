...
An adversary is using named pipe impersonation or token manipulation techniques such as Meterpreter's GetSystem command to escalate privileges from a local administrator context to NT AUTHORITY\SYSTEM on a compromised Windows host.

| Hunt # | Hypothesis | Tactic | Notes | Tags | Submitter |
|--------|-----------|--------|-------|------|-----------|
| H106 | An adversary is using named pipe impersonation or token manipulation techniques such as Meterpreter's GetSystem command to escalate privileges from a local administrator context to NT AUTHORITY\SYSTEM on a compromised Windows host. | Privilege Escalation | Look for creation of short-lived Windows services with random names, named pipe creation by non-standard processes, and token impersonation API calls (ImpersonateNamedPipeClient, DuplicateTokenEx). Sysmon Event IDs 17/18 (pipe created/connected) and 1 (process creation) with ParentUser != ChildUser are key data sources. | #privilege_escalation #T1134_005 #T1134 #named_pipe #getsystem #token_impersonation | [Lauren Proehl](https://x.com/jotunvillur) |
