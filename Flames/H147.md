---
category: Flames
hypothesis: Threat actors deliver macOS infostealers via ClickFix lures that trick
  users into pasting a base64-encoded shell pipeline into Terminal, decoding into
  a curl-piped osascript or sh payload.
id: H147
notes: 'macOS only — Microsoft Threat Intelligence (May 2026) reports SHub Stealer
  / AMOS / Macsync delivered via fake macOS-utility Squarespace, Medium, and Craft
  pages. Detection: ESF subscribers (ES_EVENT_TYPE_NOTIFY_EXEC) for process trees
  where `Terminal.app` → `zsh`/`bash` → (`base64 -D` or `base64 --decode`) piped into
  `sh`/`zsh`/`osascript`; Unified Log predicate `process == "osascript" AND parentProcess
  CONTAINS "curl"`; KQL on Defender for Endpoint macOS — verbatim from Microsoft:
  `DeviceNetworkEvents | where InitiatingProcessCommandLine has_any ("loader.sh?build=","payload.applescript?build=")`
  and `DeviceProcessEvents | where ProcessCommandLine has_all("curl","POST","txid","osascript","bmodule","max-time")`.
  Hunt for `/tmp/shub_*` and `/tmp/<random_id>/` directory creation. Cross-reference
  T1204.004 (User Execution: Malicious Copy and Paste) — the user is the delivery
  mechanism.'
submitter:
  link: https://x.com/jotunvillur
  name: Lauren Proehl
tactics:
- Execution
tags:
- execution
- macos
- clickfix
- shub_stealer
- amos
- osascript
techniques:
- T1204.004
- T1059.002
---

## Summary

Threat actors run macOS ClickFix lures that instruct users to copy a base64 blob into Terminal, where the decoded command pipes a curl-fetched payload into osascript or sh — establishing in-memory infostealer execution (SHub Stealer, AMOS/Macsync) without writing the script to disk.

## Why
- ClickFix is the dominant macOS initial-access vector in 2026 — Jamf, Datadog, Microsoft, and Hunt.io have all published on it within the past quarter, and the technique works because it converts the user into an unsigned-binary execution path that bypasses Gatekeeper and codesign verification entirely
- The curl|base64|osascript pipeline is fileless on the disk (no .scpt, no .sh dropped), so file-based AV/EDR signatures will not catch it — process telemetry on the pipeline shape and the unusual parent-child chain is the only durable detection
- Microsoft's KQL queries are public and ready to deploy, but the underlying behavior generalizes: any future macOS lure that pivots to `curl ... | osascript` will trigger this hunt regardless of campaign branding
- Each of the three identified payloads (SHub, AMOS, Macsync) targets keychain databases, browser credential stores, and crypto wallets — early detection at the execution stage prevents irreversible credential and wallet seed-phrase loss

## References
- [MITRE ATT&CK T1059.002 - Command and Scripting Interpreter: AppleScript](https://attack.mitre.org/techniques/T1059/002/)
- [Microsoft Threat Intelligence - ClickFix Campaign Uses Fake macOS Utilities Lures to Deliver Infostealers](https://www.microsoft.com/en-us/security/blog/2026/05/06/clickfix-campaign-uses-fake-macos-utilities-lures-deliver-infostealers/)
- [MITRE ATT&CK T1204.004 - User Execution: Malicious Copy and Paste](https://attack.mitre.org/techniques/T1204/004/)
- [Jamf Threat Labs - ClickFix Malware Uses macOS Script Editor to Deliver Atomic Stealer](https://www.jamf.com/blog/clickfix-macos-script-editor-atomic-stealer/)
- [Hunt.io - ClickFix on macOS: AppleScript Stealer, Terminal Phishing, and C2 Infrastructure](https://hunt.io/blog/macos-clickfix-applescript-terminal-phishing)
- [Datadog Security Labs - Tech Impersonators: ClickFix and macOS Infostealers](https://securitylabs.datadoghq.com/articles/tech-impersonators-clickfix-and-macos-infostealers/)
- [Microsoft Community Hub - Hunting Infostealers - macOS Threats](https://techcommunity.microsoft.com/blog/microsoftsecurityexperts/hunting-infostealers---macos-threats/4494435)
